...
more memory than other four tools, specifically, it brings 1.8\% more memory
overhead when compared with vanilla S2E. This is because we run both fuzzer and
symbolic execution in the system.
...
\midrule
		...
		LibFuzzer	& llvm 3.8	& Y & Y & Y & Y & N & N & N & N & N & 4 & 481177.6\\
...
% Add explaination why we took results directly from other paper
Note that since \cite{dolan2016lava} does not statement about the details of 
...
of VUzzer and \prototype in this table. Since VUzzer runs only on 32-bit system, 
we re-executed it within a new environment but running Linux Ubuntu 14.04 X86 
with the same cores and memory as our experimental setup. 
