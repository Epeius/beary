...
$256^{nbytes}$ states, causing state explosion. The bug nested at line 26 
...
face of state explosion. For example, suppose \texttt{nbytes} is 4, then 
the worst case is that the bug can only be triggered after all 
$256^4=4294967296$ states are scheduled.

There are different approaches for handling state explosion caused by 
...
Obviously, the full symbolic memory model may cause state explosion 
...
So even though lazy forking can ease the state explosion problem, 
...
is another common cause of state explosion since its loop times may 
range from 0 to infinite theoretically. 
Even though the hybrid testing method can ease state explosion, the 
...
\lstinputlisting[label={RE-SLB}, language=C,style=c,caption={A motivating 
	example to demonstrate state explosion raised by symbolic loops.}]
{codes/example-SLB.c} 
...
raw data from the \texttt{packet} at line 4.
 ...
 will result in state explosion.
...
to ease the state explosion problem due to symbolic loops.
...
that all the loop buckets will be covered without causing state explosion.
